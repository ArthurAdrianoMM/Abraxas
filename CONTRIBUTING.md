...
Everything a maintainer or contributor needs: how to build, how the typed
bindings are generated, and how a release is cut. If you only want to *use*
Abraxas, the [README](README.md) is the right place.
...
[`model-catalog/catalog.json`](model-catalog/catalog.json) and is published to
GitHub Pages by [`.github/workflows/publish-catalog.yml`](.github/workflows/publish-catalog.yml),
served from `https://arthuradrianomm.github.io/Abraxas/catalog.json`.

Adding or updating a model is a catalog commit, not an app release — clients pick
up the change on their next fetch. Each entry carries the Hugging Face URL, the
SHA-256 the download is verified against, the file size, the RAM/VRAM floors used
for the compatibility classification, and the chat template to apply.
...
The installers are **not** code-signed, so each OS warns that the developer is
unknown. The user-facing wording of the workaround ships inside the release body
itself, from [`.github/RELEASE_NOTES.md`](.github/RELEASE_NOTES.md) — edit that
file when the instructions need to change.
...
[CLAUDE.md](CLAUDE.md) holds the full project context — vision, stack rationale,
architecture decisions and the phased roadmap. Architecture Decision Records live
